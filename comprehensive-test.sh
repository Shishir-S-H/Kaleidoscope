...
docker exec kaleidoscope-redis-1 redis-cli -a ${REDIS_PASSWORD} XGROUP CREATE es-sync-queue es-sync-group $ MKSTREAM >/dev/null 2>&1 || true
docker exec kaleidoscope-redis-1 redis-cli -a ${REDIS_PASSWORD} XINFO GROUPS es-sync-queue || true
...
docker exec kaleidoscope-redis-1 redis-cli -a ${REDIS_PASSWORD} XGROUP CREATE post-image-processing content-moderation-group 0 MKSTREAM >/dev/null 2>&1 || true
docker exec kaleidoscope-redis-1 redis-cli -a ${REDIS_PASSWORD} XGROUP CREATE post-image-processing image-tagger-group 0 MKSTREAM >/dev/null 2>&1 || true
docker exec kaleidoscope-redis-1 redis-cli -a ${REDIS_PASSWORD} XGROUP CREATE post-image-processing scene-recognition-group 0 MKSTREAM >/dev/null 2>&1 || true
docker exec kaleidoscope-redis-1 redis-cli -a ${REDIS_PASSWORD} XGROUP CREATE post-image-processing image-captioning-group 0 MKSTREAM >/dev/null 2>&1 || true
docker exec kaleidoscope-redis-1 redis-cli -a ${REDIS_PASSWORD} XGROUP CREATE post-image-processing face-recognition-group 0 MKSTREAM >/dev/null 2>&1 || true
echo "Consumer groups created/verified"
docker exec kaleidoscope-redis-1 redis-cli -a ${REDIS_PASSWORD} XINFO GROUPS post-image-processing || true
...
docker exec kaleidoscope-redis-1 redis-cli -a ${REDIS_PASSWORD} XADD post-image-processing "*" mediaId 77777 postId 77777 mediaUrl "https://github.githubassets.com/images/modules/logos_page/GitHub-Mark.png" uploaderId 101 correlationId "test-77777"
docker exec kaleidoscope-redis-1 redis-cli -a ${REDIS_PASSWORD} XADD post-image-processing "*" mediaId 66666 postId 66666 mediaUrl "https://picsum.photos/seed/picsum/400/300" uploaderId 101 correlationId "test-66666"
...
docker exec kaleidoscope-redis-1 redis-cli -a ${REDIS_PASSWORD} XREAD STREAMS ml-insights-results 0 | grep -A12 -E "77777|66666" || echo "No results found for mediaId 77777/66666"

echo ""
echo -e "${BLUE}Step 6: Checking Face Detection Results${NC}"
docker exec kaleidoscope-redis-1 redis-cli -a ${REDIS_PASSWORD} XREAD STREAMS face-detection-results 0 | grep -A12 -E "77777|66666" || echo "No face detection results for mediaId 77777/66666"

echo ""
echo -e "${BLUE}Step 7: Trigger Post Aggregation${NC}"
docker exec kaleidoscope-redis-1 redis-cli -a ${REDIS_PASSWORD} XADD post-aggregation-trigger "*" postId 77777 action aggregate correlationId "test-77777"
docker exec kaleidoscope-redis-1 redis-cli -a ${REDIS_PASSWORD} XADD post-aggregation-trigger "*" postId 66666 action aggregate correlationId "test-66666"
sleep 5

echo ""
echo -e "${BLUE}Step 8: Check Post Aggregation Results${NC}"
docker exec kaleidoscope-redis-1 redis-cli -a ${REDIS_PASSWORD} XREAD STREAMS post-insights-enriched 0 | grep -A12 -E "77777|66666"

echo ""
echo -e "${BLUE}Step 9: Stream Statistics${NC}"
echo "ML Insights Stream:"
docker exec kaleidoscope-redis-1 redis-cli -a ${REDIS_PASSWORD} XINFO STREAM ml-insights-results | grep -E "length|entries"
echo ""
echo "Face Detection Stream:"
docker exec kaleidoscope-redis-1 redis-cli -a ${REDIS_PASSWORD} XINFO STREAM face-detection-results | grep -E "length|entries"
echo ""
echo "Post Insights Enriched Stream:"
docker exec kaleidoscope-redis-1 redis-cli -a ${REDIS_PASSWORD} XINFO STREAM post-insights-enriched | grep -E "length|entries"
...
docker exec -it kaleidoscope-redis-1 redis-cli -a ${REDIS_PASSWORD} XREAD BLOCK 20000 STREAMS post-insights-enriched $ || true

echo "Full read of enriched stream (for verification):"
docker exec kaleidoscope-redis-1 redis-cli -a ${REDIS_PASSWORD} XREAD STREAMS post-insights-enriched 0 | grep -A12 -E "77777|66666" || true

echo ""
echo -e "${BLUE}Step 10.1: Ensure Aggregator Group on Insights Stream${NC}"
echo "Confirm insights exist for posts:"
docker exec kaleidoscope-redis-1 redis-cli -a ${REDIS_PASSWORD} XREAD STREAMS ml-insights-results 0 | grep -A8 -E "77777|66666" || true
...
docker exec kaleidoscope-redis-1 redis-cli -a ${REDIS_PASSWORD} XGROUP CREATE ml-insights-results post-aggregator-group 0 MKSTREAM >/dev/null 2>&1 || true
docker exec kaleidoscope-redis-1 redis-cli -a ${REDIS_PASSWORD} XINFO GROUPS ml-insights-results || true
...
docker exec kaleidoscope-redis-1 redis-cli -a ${REDIS_PASSWORD} XINFO STREAM post-aggregation-trigger | grep -E "length|entries|groups|last-generated-id" || true
